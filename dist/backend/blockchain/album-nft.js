...
export class AlbumNFTMinter {
    ...
    async createCollection(collectionName, description) {
        ...
        const unsubscribe = await tx.signAndSend(this.account.address, { signer: injector.signer }, ({ status, txHash, dispatchError }) => {
            if (!txHashResult) {
     ...
     */
    async waitForCollection(collectionId, timeoutMs = 60000) {
        if (!this.api)
            throw new Error("API not initialized");
     ...
     */
    async mintAlbumNFT(collectionId, albumData) {
        if (!this.api)
            throw new Error("API not initialized");
        if (!this.account)
            throw new Error("Wallet not connected");
        ...
        const injector = await web3FromAddress(this.account.address);
        let mintTxHashResult = null;
        const mintUnsubscribe = await mintTx.signAndSend(this.account.address, { signer: injector.signer }, ({ status, txHash, dispatchError }) => {
            if (!mintTxHashResult) {
    ...
    async uploadAlbumMetadata(album) {
    ...
    async getCollectionNFTs(collectionId, ownerAddress) {
        if (!this.api)
            throw new Error("API not initialized");
        ...
        for (let itemId = 0; itemId < totalItems; itemId++) {
            ...
            if (itemDetails && itemDetails.toString() !== "") {
                ...
                if (itemOwner === owner) {
                    ...
                    const metadataData = metadata && metadata.toString() !== "" ? metadata.data?.toUtf8() : undefined;
                    nfts.push({
                        itemId,
                        owner: itemOwner,
...
export async function createMasterAlbumCollection(label) {
    const minter = new AlbumNFTMinter();
    await minter.initialize();
    await minter.connectWallet();
    return await minter.createCollection(`${label} Master CD NFTs`, `A collection of Master CD NFTs created by ${label} via Aurora.`);
}
