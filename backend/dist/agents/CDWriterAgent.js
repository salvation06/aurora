    ...
    async execute(contentBundle) {
        ...
        try {
            ...
            try {
                ...
                const { stdout, stderr } = await execAsync(`hdiutil makehybrid -o "${isoPath}" -iso -joliet -default-volume-name "${contentBundle.volumeLabel}" "${workDir}"`);
                ...
                try {
                    const { stdout, stderr } = await execAsync(`mkisofs -o "${isoPath}" -V "${contentBundle.volumeLabel}" -J -R "${workDir}"`);
                    isoLog = `ISO created successfully at ${isoPath}\n${stdout}`;
